...
def get_cached_catalog():
    ...
    if CACHE_PRODUTOS is None or (agora - ULTIMA_ATUALIZACAO) > TEMPO_CACHE:
        ...
        logging.info(f"==> Cache atualizado. {len(CACHE_PRODUTOS) if CACHE_PRODUTOS else 0} produtos recarregados.")
...
@app.route("/")
def index():
    produtos = get_cached_catalog() or []
    
    # Debug para visualizar na consola do servidor/Render
    print(f"\n--- [DEBUG ROTA INDEX] ---")
    print(f"Total de produtos obtidos: {len(produtos)}")
    if produtos:
        print(f"Exemplo do 1º Produto: {produtos[0]}")
    else:
        print("[ALERTA] load_catalog() retornou lista VAZIA!")
    print("---------------------------\n")
...
def api_produtos():
    return jsonify({"produtos": get_cached_catalog() or []})
...
def admin_dashboard():
    ...
    produtos = load_catalog() or []
    sheet_name = getattr(Config, 'SHEET_ORDERS', 'Pedidos')
    pedidos = get_orders(sheet_name) or []
